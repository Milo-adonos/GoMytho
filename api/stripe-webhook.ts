...
async function syncSupabaseUserFromPayload(
  ...
  if (emailNorm) {
    ...
    if (r1.data?.length) return
    const r2 = await supabase.from('users').update(row).eq('stripe_payment_email', emailNorm).select('id')
    if (r2.data?.length) return
  }
  if (cid) {
    await supabase.from('users').update(row).eq('stripe_customer_id', cid)
  }
}
