...
export default function Login() {
  ...
  // Récupération avec Google : pour les anciens clients qui se sont
  // inscrits via Google (pas de mot de passe). On stocke l'email Stripe
  // en sessionStorage, on lance OAuth, AuthCallback lit le flag et lance
  // le claim automatiquement.
  const handleClaimWithGoogle = async () => {
    const stripeEmail = (claimStripeEmail || '').trim().toLowerCase()
    if (!stripeEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(stripeEmail)) {
      setClaimError(
        "Indique d'abord l'email avec lequel tu as payé sur Stripe (champ 3) avant de cliquer sur Google.",
      )
      return
    }
    ...
    setClaimSuccess('')
    try {
      sessionStorage.setItem('gomytho_claim_stripe_email', stripeEmail)
      const { error: oauthErr } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
      ...
      const e = err as { message?: string }
      setClaimError(e.message || 'Connexion Google impossible.')
  ...
  const handleClaimSubmit = async (e: React.FormEvent) => {
    ...
    // Validation : email Stripe est toujours obligatoire
    const stripeEmail = (claimStripeEmail || '').trim().toLowerCase()
    if (!stripeEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(stripeEmail)) {
      setClaimError("Indique l'email avec lequel tu as payé sur Stripe.")
      return
    }
    // Validation : email + password obligatoires pour ce flux (le flux
    // Google passe par handleClaimWithGoogle).
    if (!claimEmail || !claimPwd || claimPwd.length < 6) {
      setClaimError(
        "Renseigne ton email et un mot de passe (min. 6 caractères) — ou utilise « Récupérer avec mon compte Google ».",
      )
      return
    }
                ...
                >
                  <div className="px-6 pb-6 pt-2">
                    ...
                    <p className="text-xs text-text-secondary mb-4 leading-relaxed">
                      ...
                      est bien enregistré. Indique l'email Stripe ci-dessous, puis
                      choisis ta méthode de connexion — on relie automatiquement.
                    </p>

                    <form onSubmit={handleClaimSubmit} className="space-y-4">
                      {/* Email Stripe : champ commun aux deux méthodes */}
                      <div>
                        <label className="block text-xs font-semibold mb-1.5 text-text-secondary uppercase tracking-wide">
                          Email utilisé sur Stripe pour payer
                        </label>
                        <input
                          type="email"
                        ...
                        <p className="text-[11px] text-text-secondary mt-1">
                          Peut être différent de ton email de connexion (Apple Pay, alias…).
                        </p>
                      </div>

                      {/* Option A : Google (sans mot de passe) */}
                      <button
                        type="button"
                        onClick={handleClaimWithGoogle}
                        disabled={claimLoading}
                        className="w-full flex items-center justify-center gap-3 bg-primary-bg border-2 border-lime/30 rounded-xl px-4 py-2.5 text-text-primary font-semibold hover:border-lime/60 transition-all text-sm"
                      >
                        ...
                        Récupérer avec mon compte Google
                      </button>
                      ...
                      <div className="relative my-2">
                        ...
                        <div className="relative flex justify-center text-xs">
                          <span className="px-3 bg-secondary-bg/60 text-text-secondary">
                            ou avec un compte email + mot de passe
                          </span>
                        </div>
                      </div>
                      ...
                      <Button type="submit" disabled={claimLoading} size="md" fullWidth>
                        {claimLoading ? 'Vérification…' : 'Récupérer avec email + mot de passe'}
                      </Button>
                    </form>
                  </div>
                </motion.div>
