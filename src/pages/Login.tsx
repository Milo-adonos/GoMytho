...
// ─── Page Connexion (version simplifiée) ─────────────────────────────────
//
// Flux unique :
//   1. signInWithPassword / signInWithOAuth(google)
//   2. Lecture du profil DB
//   3. Si abonné → /makemytho
//   4. Sinon → /choixoffre (l'utilisateur reprend son parcours d'abo
//      avec le MÊME compte ; pas de signOut, pas de message anxiogène).
//
// Plus de récupération par email Stripe (« claim ») ni de fallback
// resolve-by-stripe : avec le flux d'inscription AVANT paiement, le
// webhook Stripe lie toujours par user.id, donc la DB est la source
// de vérité.
...
export default function Login() {
  ...
  const goAfterLogin = async (userId: string) => {
    ...
    if (hasPaidGoMythoAccess(profile)) {
      ...
      // sera réutilisée pour la liaison Stripe).
